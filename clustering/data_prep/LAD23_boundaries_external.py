...
def read_in_census_21(loaded_config):
    """
    This function reads in 2021 Census population by single year of age for each LAD, renames columns to remove spaces and drops unnecessary columns. No parameters needed.
...
def read_in_area_file(loaded_config): 
    """
    This function reads in LAD areas in hectares, drops unnecessary columns and creates two new fields: total area in square kilometers and total area in square meters. No parameters needed. 
    """
...
def convert_LAD22_to_LAD23(loaded_config, metric, start_col, denominator = "population", end_col=None):
    """
    This function can be used to calculate values for the LAD that were created as a result of the 2023 Local Authority boundary changes (North Yorkshire, Somerset, Cumberland and Westmorland and Furness). The function imputes new values for a new local authority (e.g. North Yorkshire) by taking the data for its constituent former LAD (e.g. Craven, Hambleton, etc.) and "weighing" it based on population or area. 
    ...
    denominator = name of dataset to be used as denominator for conversion, defaults to population. Options are:
        ...
        2.area = Land area of LAD in hectares (AREAEHECT, position 2), square km (AREAKM, position 3), or square meters (AREASQM, position 4) - (download hectares only data from: https://geoportal.statistics.gov.uk/datasets/235c70d40c494361bd6b0ddaebdf0bad/about). 
   start_col = Position of the column from denominator dataset to be used as the denominator for new values calculations. For Census total population (no age restrictions) or for total area start_col should be set to 2. Generally, to get the position of the column for an age you should add 2 to the required age (e.g. "aged_18_years" will be in position 20).
    end_col = defaults to None, but should always be set if using a range of columns. The position of the end column in the range of columns to sum for the denominator (e.g. if population 18-24 is needed, the end_col should be set to the census col "aged_24_years", which means that the parameter should be set to 26 (24+2). 
    """
    import numpy as np
    ...
    if (metric["AREACD"]=="E10000023").any():
        ...
        den_NY = den[(den['AREACD'] == 'E07000163') | (den['AREACD'] == 'E07000164') | (den['AREACD'] == 'E07000165')| (den['AREACD'] == 'E07000166') | (den['AREACD'] == 'E07000167')| (den['AREACD'] == 'E07000168') | (den['AREACD'] == 'E07000169')]
    ...
    if (metric["AREACD"]=="E10000027").any():
        ...
        den_somerset = den[(den['AREACD'] == 'E07000187') | (den['AREACD'] == 'E07000188') | (den['AREACD'] == 'E07000189')| (den['AREACD'] == 'E07000246')]
    ...
    den_cumberland = den[(den['AREACD'] == 'E07000026') | (den['AREACD'] == 'E07000028') | (den['AREACD'] == 'E07000029')]
    ...
    den_wf= den[(den['AREACD'] == 'E07000030') | (den['AREACD'] == 'E07000031') | (den['AREACD'] == 'E07000027')]
    ...
    if (metric["AREACD"]=="E07000150").any():
        ...
        LAD21_north_north = metric[(metric['AREACD'] == 'E07000150') | (metric['AREACD'] == 'E07000152') | (metric['AREACD'] == 'E07000153') | (metric['AREACD'] == 'E07000156')]
        den_north_north = den[(den['AREACD'] == 'E07000150') | (den['AREACD'] == 'E07000152') | (den['AREACD'] == 'E07000153') | (den['AREACD'] == 'E07000156')]
        ...
        LAD21_west_north = metric[(metric['AREACD'] == 'E07000151') | (metric['AREACD'] == 'E07000154') | (metric['AREACD'] == 'E07000155')]
        den_west_north = den[(den['AREACD'] == 'E07000151') | (den['AREACD'] == 'E07000154') | (den['AREACD'] == 'E07000155')]
    ...
    inactive_codes= ["E07000163", "E07000164", "E07000165", "E07000166", "E07000167", "E07000168", "E07000169", "E07000187", "E07000188", "E07000189", "E07000246", "E07000026", "E07000028", "E07000029", "E07000030", "E07000031", "E07000027"]
